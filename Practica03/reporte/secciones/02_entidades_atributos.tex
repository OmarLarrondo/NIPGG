...
La cantidad disponible no es una propiedad global de \texttt{Premio}. En el
diagrama final es un atributo de la relación \texttt{Disponer}, porque el mismo
tipo de premio puede tener existencias diferentes en cada sucursal.
...
Entidad & Atributo & Dominio y caracterización \\
\midrule
Empleado & \texttt{CURP} & Identificador candidato único de la persona
   empleada. \\
 & \texttt{RFC} & Identificador candidato único, con formato de RFC. El
   diagrama subraya tanto \texttt{CURP} como \texttt{RFC}. \\
 & \texttt{nombreCompleto} & Atributo compuesto por \texttt{nombre},
   \texttt{apellidoPaterno} y \texttt{apellidoMaterno}; cadenas de texto. \\
 & \texttt{fechaNacimiento} & Fecha válida de nacimiento. \\
 & \texttt{teléfono} & Atributo multivalorado; conjunto de cadenas con formato
...
requieren un identificador distinto: heredan los identificadores
\texttt{CURP} y \texttt{RFC} de la superentidad. En la versión final del
diagrama, \texttt{Juego} se
